...
QWidget* MyDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    else if (index.column() != ColumnNames::PEN_STYLE) // Индекс не соответствует колонке «стиля пера»
        return QStyledItemDelegate::createEditor(parent, option, index);
...
bool MyDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    else if (index.column() != ColumnNames::PEN_COLOR) // Индекс не соответствует колонке «стиля пера»
        return QStyledItemDelegate::editorEvent(event, model, option, index);
...
{
    if (index.column() == ColumnNames::PEN_STYLE) // Если обрабатывается колонка со стилем пера
    {
        QStyleOptionComboBox styleOptionCombo;
        int style = index.model()->data(index, Qt::EditRole).toInt();
        QString text = styles[style];
